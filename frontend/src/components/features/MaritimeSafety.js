...
const MaritimeSafety = () => {
  ...
  useEffect(() => {
    ...
    if (autoRefresh) {
      intervalRef.current = setInterval(() => {
        if (location) {
          fetchEnvironmentalData(location.lat, location.lon, false);
        }
      }, 30000);
    }
  ...
  const initializeMap = (lat, lon) => {
    // Initialize VesselFinder map with user's location
    if (mapRef.current) {
      // Clear existing map
      mapRef.current.innerHTML = '';
      
      // Create script elements for the VesselFinder map
    ...
    <div className="space-y-4 p-4 max-w-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600">Live location tracking & environmental conditions</p>
        </div>
      ...
      {environmentalData && (
        <Card className={`${getStatusColor(environmentalData.risk_analysis.overall_risk_level)} border-2`}>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-bold text-lg">{environmentalData.risk_analysis.risk_message}</h3>
                <p className="text-sm">
                  Risk Level: {environmentalData.risk_analysis.overall_risk_level}
                </p>
              </div>
              <Shield className={`w-8 h-8 ${
                environmentalData.risk_analysis.overall_risk_level.includes('DANGER') ? 'text-red-600' :
                environmentalData.risk_analysis.overall_risk_level === 'CAUTION' ? 'text-orange-600' :
                'text-green-600'
              }`} />
            </div>
          </CardContent>
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Your current position - No vessel tracking (Privacy Mode)
          </CardDescription>
        </CardHeader>
        <CardContent className="p-0">
          ...
          >
            {/* VesselFinder map will be injected here */}
          </div>
        </CardContent>
      </Card>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 rounded-lg bg-blue-50 border border-blue-200">
                ...
                <div className="text-lg font-bold text-blue-700">
                  {environmentalData.environmental_data.sea_surface_temp_c.toFixed(1)}°C
                </div>
              </div>
              ...
              <div className="p-3 rounded-lg bg-green-50 border border-green-200">
                ...
                <div className="text-lg font-bold text-green-700">
                  {environmentalData.environmental_data.wind_speed_knots.toFixed(1)} kts
                </div>
              </div>
              ...
              <div className="p-3 rounded-lg bg-purple-50 border border-purple-200">
                ...
                <div className="text-lg font-bold text-purple-700">
                  {environmentalData.environmental_data.ocean_current_knots.toFixed(1)} kts
                </div>
              </div>
              ...
              <div className="p-3 rounded-lg bg-orange-50 border border-orange-200">
                ...
                <div className="text-lg font-bold text-orange-700">
                  {Math.round(environmentalData.risk_analysis.rogue_wave_probability * 100)}%
                </div>
              </div>
            </div>

            {/* Safety Recommendations */}
            {environmentalData.recommendations && environmentalData.recommendations.length > 0 && (
              <div className="mt-4">
                <h4 className="font-medium text-sm mb-2">🛡️ Safety Recommendations:</h4>
                <div className="space-y-1">
                  {environmentalData.recommendations.slice(0, 3).map((recommendation, index) => (
